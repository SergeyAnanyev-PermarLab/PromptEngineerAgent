...
## Known Limitations
These are behavioral risks that static design cannot fully resolve — they require runtime observation and calibration:
- **Input Router over-activation:** The Input Router defaults to SOP mode. If the user submits a legitimate meta-instruction (question about the spec, file edit request) and the agent routes it to the SOP anyway, the router's exclusion categories need expansion. Monitor for false-positive SOP entries.
